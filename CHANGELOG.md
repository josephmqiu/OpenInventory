...
### Docs
- The README now opens with a product showcase — screenshots and demo GIFs of the desktop app (issue & batch-issue, inventory search/filter, executive period reports) and the mobile QR lookup — with the developer and build documentation moved below. Media is regenerated by scripts under `scripts/media/`.

### For contributors
- Fixed `scripts/seed-demo-data.ts`, which crashed partway through seeding because it still inserted into the `low_stock_alerts.channel_summary` column removed by an earlier migration. Added `SEED_DB_PATH`, `SEED_ASSUME_YES`, and `SEED_END_DATE` env overrides so the browser-preview (`.dev-data`) database can be seeded with current demo data without touching real app data.
